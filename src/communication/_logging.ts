...
export class ConsoleAgentCommunicationLogger
    extends BaseAgentCommunicationLogger {
    private static readonly REMEMBERED_HANDOFF_MESSAGE_LIMIT = 256;
    private readonly theme: TerminalTheme = new TerminalTheme();
    private readonly receivedHandoffMessageIds =
        new Set<AgentCommunicationMessageId>();
    private readonly sentHandoffMessageStatuses =
        new Map<
    ...
    log(event: AgentCommunicationLog): void {
        switch (event.event) {
            ...
            case "message-sent":
                ...
                const sentMessage = this.formatSentMessage(
                    event.message,
                    event.peerId,
                    event.status,
                );
            ...
            case "message-received":
                if (ConsoleAgentCommunicationLogger.isHandshakeMessage(
                    event.message,
                ) || this.isRepeatedHandoffReception(event.message)) {
                    return;
                }
                const receivedMessage = this.formatReceivedMessage(
                    event.message,
                    event.peerId,
                );
    ...
    private isRepeatedHandoffSend(
        ...
        const previousStatus = this.sentHandoffMessageStatuses.get(
            message.messageId,
        );
    ...
    private isRepeatedHandoffReception(
        message: AgentCommunicationMessage,
    ): boolean {
        if (!ConsoleAgentCommunicationLogger.isHandoffMessage(message)) {
            return false;
    ...
    private static isHandoffMessage(
        message: AgentCommunicationMessage,
    ): boolean {
    ...
    private trimRememberedMessages<T>(messages: Set<T> | Map<T, unknown>): void {
        if (
            messages.size
