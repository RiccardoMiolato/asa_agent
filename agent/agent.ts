...
import { getDirection, getNextParcel, getClosestDeliveringCell } from "./utils.js";
...
class Agent {
    ...
    async makeMove() {
        for (const parcel of beliefs.parcels.values()) {
            ...
            if(beliefs.state == 0 && beliefs.target_parcel == undefined && parcel.carriedBy == null){
        ...
        if(this.position.x % 1 === 0 && this.position.y % 1 === 0 && beliefs.target_pos != undefined && beliefs.followed_path != null) {
            if (beliefs.followed_path.length > 0) {
                ...
                if(beliefs.target_parcel != undefined){
                    await socket.emitPickup();
