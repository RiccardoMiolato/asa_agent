    ...
    buildClusterRevisitActions(context, oldestClusters) {
        ...
        for (const cluster of [...remoteClusters, ...localClusters]) {
            ...
            for (const destination of destinations) {
                const movementPath = context.pathfinder.findMovementPath(context.gameMap, context.agentPosition, destination, context.crates);
                ...
                const pathWithMovableCrates = context.pathfinder.findMovementPath(context.gameMap, context.agentPosition, destination, new Map());
    ...
    buildCoveragePlan(context, cluster, requiredCellKeys, crates = context.crates) {
        ...
        while (uncoveredKeys.size > 0) {
            ...
            for (const candidate of this.rankCoverageCandidates(candidates, cursor, uncoveredKeys, cluster.cells, context.observationDistance)) {
                const movementPath = context.pathfinder.findMovementPath(context.gameMap, cursor, candidate.destination, crates);
